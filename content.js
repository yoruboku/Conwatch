...
function getMetadata(video) { // video can be null
    ...
    // Strict Regex: Matches "S01E01", "Season 1", "Episode 1", "Ep 1"
    const epRegex = /S(\d+)E(\d+)|Season\s(\d+)|Episode\s(\d+)|Ep\.?\s?(\d+)/i;

    // A. Try to find episode string
    if (video) {
        const bodyText = document.body.innerText.substring(0, 5000); 
        const epMatch = (title + " " + bodyText).match(epRegex);
        if (epMatch) episode = epMatch[0];
    }

    // B. GENERATE ROBUST SERIES ID (Multi-layered)
    
    // Layer 1: Check Metadata Tags (Standard for many streaming sites)
    const metaSeries = document.querySelector('meta[property="og:video:series"], meta[name="series_name"]');
    if (metaSeries && metaSeries.content) {
        seriesId = hostname + "||meta||" + metaSeries.content.trim().toLowerCase();
    }

    // Layer 1.5: Query Parameter Logic (For sites like rivestream.org/watch?id=123...)
    if (!seriesId) {
        const urlParams = new URLSearchParams(window.location.search);
        // Common keys used for Series/Show IDs
        const idKeys = ['id', 'show_id', 'series_id', 'bangumi_id', 'playlist_list']; 
        
        for (const key of idKeys) {
            if (urlParams.has(key)) {
                // We found a stable ID! e.g. "rivestream.org||query||/watch?id=62560"
                seriesId = `${hostname}||query||${window.location.pathname}?${key}=${urlParams.get(key)}`;
                break;
            }
        }
    }

    // Layer 2: URL Path Logic (e.g., site.com/watch/show-name/ep1 -> /watch/show-name/)
    if (!seriesId && window.location.pathname.length > 5) {
        const pathSegments = window.location.pathname.split('/').filter(Boolean);
        if (pathSegments.length > 1) {
            // Remove the last segment (usually the episode ID/slug)
            pathSegments.pop();
            const parentPath = pathSegments.join('/');
            seriesId = hostname + "||path||" + parentPath;
        }
    }

    // Layer 3: Title Parsing Fallback
    if (!seriesId) {
        const epMatch = title.match(epRegex);
        if (epMatch) {
            const cleanTitle = title.replace(epRegex, '').replace(/[-:|]\s*$/, '').trim();
            if (cleanTitle.length > 1) {
                seriesId = hostname + "||title||" + cleanTitle.toLowerCase();
            }
        }
    }
    ...
    return { title, episode, seriesId, thumbnail, favicon };
}
